    ...
    <DmData ref="DmData" @init="fetchList">
      ...
      <DmTable :loading="loading" min-height>
        <el-table :data="list" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="用户名称" prop="package_name" min-width="150" />
          <el-table-column label="应用状态" prop="status" min-width="150">
            <template slot-scope="{row}">{{row.status === 1 ?"启用":'禁用'}}</template>
          </el-table-column>
          <el-table-column label="创建时间" prop="created_at" min-width="150" />
          <el-table-column label="更新时间" prop="updated_at" min-width="150" />
          <!-- <el-table-column
            label="AccessKey"
            min-width="150"
          >
          ...
          <el-table-column v-if="false" label="到期时间" min-width="100">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" width="200" align="right">
            <template slot-scope="scope">
              ...
              <el-button type="text" @click="$refs.config.handleOpen()">管理</el-button>
