  ...
  methods: {
    ...
    async handleAction(type) {
      this.$confirm('确认操作?', '提示', {
        ...
        if (type === 'open') {
          await this.Fetch.post('V4/firewall.policy.open', {
            ids: this.selectionId
          });
        } else if (type === 'stop') {
          await this.Fetch.post('V4/firewall.policy.stop', {
            ids: this.selectionId
          });
        } else if (type === 'delete') {
          await this.Fetch.post('V4/firewall.policy.delete', {
            ids: this.selectionId
          });
        }
